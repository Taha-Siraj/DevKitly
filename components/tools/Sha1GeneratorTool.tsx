...
export default function Sha1GeneratorTool() {
  const [inputText, setInputText] = useState("DevKitly SHA1");
  ...
  useEffect(() => {
    ...
    const sha1Val = CryptoJS.SHA1(inputText).toString();
    setHash(sha1Val);
  }, [inputText, fileName]);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (ab) {
        ...
        const sha1Val = CryptoJS.SHA1(wordArray).toString();
        setHash(sha1Val);
        toast("File SHA-1 checksum calculated!");
      }
    };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="flex flex-col space-y-2">
          ...
          <textarea
            ...
            disabled={!!fileName}
            placeholder="Type or paste your text to calculate SHA-1 checksum..."
        ...
        <div className="flex flex-col space-y-2">
          <div className="flex justify-between items-center h-5">
            <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">SHA-1 Hex Digest</span>
            {hash && (
              <button
                onClick={handleCopy}
