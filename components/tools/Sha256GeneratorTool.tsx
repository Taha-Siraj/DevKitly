...
export default function Sha256GeneratorTool() {
  const [inputText, setInputText] = useState("DevKitly SHA256");
  ...
  useEffect(() => {
    ...
    const sha256Val = CryptoJS.SHA256(inputText).toString();
    setHash(sha256Val);
  }, [inputText, fileName]);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (ab) {
        ...
        setHash(sha256Val);
        toast("File SHA-256 checksum calculated!");
      }
    };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="flex flex-col space-y-2">
          ...
          <textarea
            ...
            disabled={!!fileName}
            placeholder="Type or paste your text to calculate SHA-256 checksum..."
        ...
        <div className="flex flex-col space-y-2">
          <div className="flex justify-between items-center h-5">
            <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">SHA-256 Hex Digest</span>
            {hash && (
              <button
                onClick={handleCopy}
