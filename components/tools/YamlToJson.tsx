...
const SAMPLE_YAML = `app: devkitly
version: 1.0.0
secure: true
services:
  - name: encoder
...
export default function YamlToJson() {
  ...
  const handleDownload = () => {
    if (!output) return;
    downloadAsFile("config.json", output, "application/json");
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="flex flex-col space-y-2">
          <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">YAML Input</span>
          <textarea
            value={input}
